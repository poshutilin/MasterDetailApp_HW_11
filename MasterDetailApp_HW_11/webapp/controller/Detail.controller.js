sap.ui.define([
	...
	OverflowToolbarButton, OverflowToolbarToggleButton, ObjectStatus, Button, Filter, FilterOperator, Fragment, MessageBox, Dialog, Control) {
	"use strict";
	...
	return BaseController.extend("jetcourses.MasterDetailApp.controller.Detail", {
		...
		_loadTable: function(sEntity) {
			...
			this._oTable.addColumn(new sap.ui.table.Column({
				template: new sap.m.Input({
					value: "{"+sEntity.slice(0, -1)+"Text"+"}"
				}),
				label: new sap.m.Label({
					text: sEntity.slice(0, -1)+"Text"
				}),
				customData: [
					new sap.ui.core.CustomData({
						key: "p13nData",
						value: {
							"columnKey": sEntity.slice(0, -1)+"Text",
							"leadingProperty": sEntity.slice(0, -1)+"Text",
							"columnIndex": "2"
						}
					})
				]
		...
		onPressCreate: function(oEvent) {
			...
			const entityToCommonItem = {
				"zjblessons_base_Groups": { title: i18nBundle.getText("tGroups"), field: "GroupText", properties: { GroupID: "", Version: "A", Language: "RU" } },
				"zjblessons_base_SubGroups": { title: i18nBundle.getText("tSubGroupText"), field: "SubGroupText", properties: { SubGroupID: "", Version: "A", Language: "RU" } },
				"zjblessons_base_Plants": { title: i18nBundle.getText("tPlantText"), field: "PlantText", properties: { PlantID: "", Version: "A", Language: "RU" } },
				"zjblessons_base_Regions": { title: i18nBundle.getText("tRegions"), field: "RegionText", properties: { RegionID: "", Version: "A", Language: "RU" } },
			};
			const currentItem = entityToCommonItem[entity];
			const oCommonItem = new JSONModel({
		        title: currentItem.title,
		    });
		    this.getView().setModel(oCommonItem, "commonItem");
			sap.ui.core.Fragment.load({
				name: "jetcourses.MasterDetailApp.view.CommonItem",
				controller: this
			}).then(oDialog => {
				let oContext = this.getModel().createEntry(entity, {
					properties: currentItem.properties,
				});
				
			    sap.ui.getCore().byId("commonItemSmartField").bindProperty("value", currentItem.field);
